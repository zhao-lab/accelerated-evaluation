...
AIC_record = (K_start:K_end)*0;
log_likelihood_record = (K_start:K_end)*0;
...
for K=K_start:K_end  % Number of Mixtrue
    ...
    log_likelihood_record(K - K_start + 1) = log_likelihood;
    BIC = log(N_data)*K*(3+9+1)-2*log_likelihood;
    AIC = 2*K*(3+9+1)-2*log_likelihood;
    BIC_record(K - K_start + 1) = BIC;
    AIC_record(K - K_start + 1) = AIC;
    ...
    filename=['test_fitting_result_leftTurn_K=' num2str(K)];
    save(filename,'BIC_record','AIC_record','eta_ite','mu_ite','sigma_ite');
