...
clear
fitting_data=load('fitting_result_pedestrian_K_from_3_to_21.mat','eta_ite','mu_ite','sigma_ite');
astar=[0 -7 5];
bstar=[13 6 60];
rng(123)
...
for i=1:K
    ...
    N_data_iter=100; % number of data used in each iteration
        ...
        for j=1:size(pos_comb,1);
            ...
            cvx_begin
                ...
                for k=1:N_set_0_for_dom
                    x(pos_comb(j,k)) <= S_0_for_dom(k,pos_comb(j,k));
                end
                x(1) >= astar(1);
                x(2) >= astar(2);
                x(3) >= astar(3);
